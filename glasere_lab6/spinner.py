...
def b2_handler(pin):
    ...
    tim1.init(period=60000, mode=Timer.PERIODIC, callback = lambda t: post())
...
def post():
    ...
    try:
        ...
        #data = '{"value1": 50}'
        data = ujson.dumps({"value1": str(session) + ' ||| ' + str(mpu.velocity[0]) + ' ||| '+ str(mpu.velocity[1]) + ' ||| '+ str(mpu.velocity[2]) + ' ||| ' + str(mpu.gyro()[0]) + ' ||| ' + str(mpu.gyro()[1]) + ' ||| ' + str(mpu.gyro()[2]) + ' ||| ' + str(mpu.temperature())})
        print(data)
        url = "https://maker.ifttt.com/trigger/test/with/key/ltP4ItXBGgPjS5TuNCSEMMrwaTwvbaaV2YpDqwEYxiE"
